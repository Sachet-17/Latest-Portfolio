...
const Experience = () => {
  ...
  const highlightImpact = (text) => {
    ...
    return parts.map((part, index) => {
      if (/^\d+$/.test(part) || part === '%') {
        return (
          <span key={index} className="text-purple-400 font-bold text-lg">
            {part}
          </span>
        );
      }
          ...
          {experience.map((exp, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative border border-gray-800 rounded-lg p-6 md:p-8 hover:border-purple-500/50 transition-all duration-300 bg-gray-900/80 backdrop-blur-sm hover:transform hover:scale-[1.01]">
                  {/* Header */}
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                    <div className="flex-1">
                      <h3 className="text-2xl md:text-3xl font-light mb-2 group-hover:text-purple-400 transition-colors">
                        {exp.role}
                      </h3>
                    ...
                    <Button
                      ...
                      className="self-start hover:bg-purple-500/20 transition-all duration-300 hover:scale-105"
                    >
                      {isExpanded ? (
                        <>
                    ...
                    <div className="mb-6 space-y-3 animate-fadeInUp">
                      ...
                      <ul className="space-y-3">
                        {exp.achievements.map((achievement, index) => (
                          <li
                            key={index}
                            className="flex items-start gap-3 text-gray-300 bg-gray-800/50 p-4 rounded-lg border border-gray-700/50 hover:border-purple-500/50 transition-all duration-300 hover:transform hover:translate-x-2"
                          >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {exp.technologies.map((tech) => (
                        <Badge
                          key={tech}
                          variant="outline"
                          className="text-xs px-3 py-1 bg-gray-800/50 border-gray-700 hover:bg-purple-500/20 hover:border-purple-500 transition-all duration-300 hover:scale-110"
                        >
                          {tech}
                        </Badge>
                      ))}
